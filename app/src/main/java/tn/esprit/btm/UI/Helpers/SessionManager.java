package tn.esprit.btm.UI.Helpers;

import android.content.Context;
import android.content.SharedPreferences;
import android.content.SharedPreferences.Editor;
import android.util.Log;

public class SessionManager {
    // LogCat tag
    private static String TAG = SessionManager.class.getSimpleName();

    // Shared Preferences
    SharedPreferences pref;

    Editor editor;
    Context _context;

    // Shared pref mode
    int PRIVATE_MODE = 0;

    // Shared preferences file name
    private static final String PREF_NAME = "AutohireLogin";

    private static final String KEY_IS_LOGGEDIN = "isLoggedIn";

    public SessionManager(Context context) {
        this._context = context;
        pref = _context.getSharedPreferences(PREF_NAME, PRIVATE_MODE);
        editor = pref.edit();
    }

    public void setLogin(boolean isLoggedIn,String username) {

        editor.putBoolean(KEY_IS_LOGGEDIN, isLoggedIn);
        editor.putString("mail",username);


        // commit changes
        editor.commit();

        Log.d(TAG, "User login session modified!");
    }
    public void setToken(boolean isLoggedIn,String token) {

        editor.putBoolean(KEY_IS_LOGGEDIN, isLoggedIn);
        editor.putString("token",token);


        // commit changes
        editor.commit();

        Log.d(TAG, "User login session modified!");
    }

    public String getUser() {

        return pref.getString("mail", "No name");
    }
    public int getUserByid() {

        return pref.getInt("id", 1);
    }
    public String getUserByToken() {

        return pref.getString("token", "No token");
    }

    public boolean isLoggedIn(){
        return pref.getBoolean(KEY_IS_LOGGEDIN, false);
    }
}
